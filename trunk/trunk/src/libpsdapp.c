...
#ifdef LOG_MSG
	char pLine[256];
	sprintf(pLine,"Psd2Psd(%s,%s) is called\n",PsdSrc,PsdDst);
	Log_MsgLine(LOGFILE_DEBUG,pLine);	
#endif
...
#ifdef LOG_MSG
	char pLine[256];
	sprintf(pLine,"PsdLoadFile(%s) is called\n",PsdFileName);
	Log_MsgLine(LOGFILE_DEBUG,pLine);	
#endif
...
DLL_API psd_handle  PsdLoadFile(char *PsdFileName)
{
	...
	if (status == psd_status_done)
	{
		...
		{
		#ifdef LOG_MSG
			sprintf(pLine,"open too many files!\n");
			Log_MsgLine(LOGFILE_DEBUG,pLine);
		#endif
...
#ifdef LOG_MSG
	char pLine[256];
	sprintf(pLine,"PsdGetLayers(%s) is called\n",PsdSrc);
	Log_MsgLine(LOGFILE_DEBUG,pLine);	
#endif
...
#ifdef LOG_MSG
	char pLine[256];
	sprintf(pLine,"PsdNewFile(%s,%s) is called\n",PsdFileName,bgPic);
	Log_MsgLine(LOGFILE_DEBUG,pLine);	
#endif
...
#ifdef LOG_MSG
	char pLine[256];
	sprintf(pLine,"PsdCloseFile(%d) is called\n",fHandle);
	Log_MsgLine(LOGFILE_DEBUG,pLine);	
#endif	
...
DLL_API int  PsdSaveFile(psd_handle fHandle)
{
	...
	Log_MsgLine(LOGFILE_DEBUG,pLine);		
	 ...
	 {
#ifdef LOG_MSG
	sprintf(pLine,"begin to merge layer...layer count=%d\n",context->layer_count);
	Log_MsgLine(LOGFILE_DEBUG,pLine);
...
#ifdef LOG_MSG
	char pLine[256];
	sprintf(pLine,"PsdPreview(%d,%s) is called\n",fHandle,fBmpFile);
	Log_MsgLine(LOGFILE_DEBUG,pLine);	
#endif	
	if ( (fHandle < 0)||(fHandle >= MAX_PSD_FILE_TABLE_ITEM) )
		 return -1;
...
#ifdef LOG_MSG
	char pLine[256];
	sprintf(pLine,"PsdNewLayer(%d,%s,%d,%d,%d,%d) is called\n",fHandle,sPngFileName,PosX,PosY
	...
	,unsigned char ucLayerIndex,unsigned char Transpancy)
{
...
#ifdef LOG_MSG
	char pLine[256];
	sprintf(pLine,"PsdCreateLayer(%d,%s,%d,%d,%d,%d,%d,%d) is called\n",fHandle,fPngName,PosX,PosY
	...
	{
	#ifdef LOG_MSG
		sprintf(pLine,"Error!Image can not load, image dimention should <= background!\n");
		Log_MsgLine(LOGFILE_DEBUG,pLine);
	#endif
		return -4;
	}
	...
	if (pDstPng)
	{
		...
		psd_freeif(pDstPng);
	#ifdef LOG_MSG
		sprintf(pLine,"layer image is shaped,saved as %s\n",fPngTmp);
		Log_MsgLine(LOGFILE_DEBUG,pLine);
	#endif			
...
#ifdef LOG_MSG
	char pLine[256];
	sprintf(pLine,"PsdSetLayerDirection(%d,%d,%d) is called\n",fHandle,WhichLayer,ImgDir);
	Log_MsgLine(LOGFILE_DEBUG,pLine);	
#endif		
...
#ifdef LOG_MSG
	char pLine[256];
	sprintf(pLine,"PsdSetLayerBrightness(%d,%d,%d) is called\n",fHandle,WhichLayer,Brightness);
	Log_MsgLine(LOGFILE_DEBUG,pLine);	
#endif	
...
#ifdef LOG_MSG
	char pLine[256];
	sprintf(pLine,"PsdSetLayerContrast(%d,%d,%d) is called\n",fHandle,WhichLayer,Contrast);
	Log_MsgLine(LOGFILE_DEBUG,pLine);	
#endif		 
	if ( (fHandle <0 )||(fHandle >= MAX_PSD_FILE_TABLE_ITEM) )
		return -1;
...
*/
DLL_API int  PsdZoomLayer(psd_handle fHandle,unsigned char WhichLayer, unsigned char ucZoomRate)
{
	psd_layer_record * layer=NULL;
	int ret=-1,i;
	psd_context *context = gPsdFileTable[fHandle].pContext;
...
DLL_API int PsdSetLayerColor(FILE *fp, int Layer, int AddRed,int AddGreen,int AddBlue)
{
	return 0;
}
