...
#ifdef LOG_MSG
	sprintf(pLine,"layer%d_info(top=%d,left=%d,width=%d,height=%d,transpancy=%d),ch_len=%d,ch_num=%d\n",i,layer->top,layer->left,
		...
		else
		{
			...
			*/
			...
			else if ( (layer->number_of_channels==5) )
			{
				...
				sprintf(pLine,"layer%d_mask(top=%d,left=%d,width=%d,height=%d,relative=%d)\n"
...
static psd_status psd_get_mask_info(psd_context * context)
{
	...
	//#ifdef LOG_MSG
	#if 0
		sprintf(pLine,"global layer mask length=%d\n",length);
		Log_MsgLine(LOGFILE_INFO,pLine);
	#endif
...
psd_status psd_get_layer_and_mask(psd_context * context)
{
	...
	length = psd_stream_get_int(context);
	#if 0
		sprintf(pLine,"Layer+mask length is:%d\n",length);
		Log_MsgLine(LOGFILE_INFO,pLine);
	#endif
	if(length <= 0)
		return psd_status_done;
